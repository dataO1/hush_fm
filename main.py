...
FIXED: Correct replaceTrack() usage (synchronous, not async)
"""
...
class SilentDiscoServer:
    ...
    async def _handle_play_audio(self, request: web.Request) -> web.Response:
        try:
            ...
            audio_track = PausableAudioTrack(audio_file.path)
            room.audio_track = audio_track
                    ...
                    try:
                        ...
                        if transceivers and len(transceivers) > 0:
                            ...
                            if audio_transceiver.sender:
                                # ✅ FIX: replaceTrack() is NOT async - don't await it
    ...
    async def _handle_webrtc_offer(self, request: web.Request) -> web.Response:
        try:
            ...
            # ✅ FIX: Use replaceTrack() synchronously (not async)
            room = self.rooms.get(room_id)
            if room and room.is_playing and room.current_audio:
                transceivers = peer.getTransceivers()
                if transceivers and len(transceivers) > 0:
                    audio_transceiver = transceivers[0]
                    
                    if audio_transceiver.sender:
                        new_track = PausableAudioTrack(room.current_audio.path)
                        new_track.set_paused(not room.is_playing)
                        
                        # replaceTrack() is NOT async - call it directly
                        audio_transceiver.sender.replaceTrack(new_track)
                        logger.info(f"🎵 Replaced audio track for {client_id}")
                    else:
                        logger.warning(f"⚠️  No sender for {client_id}, will add track on play")
                else:
