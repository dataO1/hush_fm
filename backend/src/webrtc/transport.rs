...
impl TransportManager {
    ...
    // Helper to get real IP or fallback
    fn get_announced_ip(&self) -> String {
        match local_ip() {
            Ok(ip) => ip.to_string(),
            Err(e) => {
                tracing::warn!("Failed to detect local IP, falling back to localhost: {}", e);
                "127.0.0.1".to_string()
            }
        }
    }
    ...
    pub async fn create_dj_transport(
        ...
        // 1. Get Real IP (e.g., 192.168.1.50)
        let announced_ip = self.get_announced_ip();
        tracing::info!("Creating DJ Transport. Announcing IP: {}", announced_ip);
